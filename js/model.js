class TravelRecommender {
  ...
  recomendar(region, pais, ciudad, tipoTurismo, presupuesto) {
    console.log("Buscando recomendaciones con:", {
      region,
      pais,
      ciudad,
      tipoTurismo,
    ...
    try {
      ...
      if (filtrados.length === 0) {
        console.log(
          "No se encontraron lugares que coincidan con los criterios"
        );
        return [];
      }
      ...
      console.log(
        `Encontrados ${filtrados.length} lugares que coinciden con los criterios`
      );
      ...
      for (let i = 0; i < filtrados.length && recomendados.length < 3; i++) {
      ...
      console.log(`Retornando ${recomendados.length} recomendaciones`);
      return recomendados;
    } catch (error) {
